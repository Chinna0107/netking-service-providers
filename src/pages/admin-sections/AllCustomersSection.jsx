...
import { MdSearch, MdEdit, MdDelete, MdPerson, MdPhone, MdEmail, MdLocationOn } from 'react-icons/md';
...
export default function AllCustomersSection() {
    ...
    <div className="customers-section">
      ...
      <div className="workflow-hero">
        <div>
          ...
          <h2>All Customers</h2>
          <p>View, search, and manage all customer accounts. Delete customers if needed (this will remove all related data).</p>
        </div>
        ...
        <div className="compact-stats-grid">
          ...
          <div className="compact-stat-card">
            <strong>{customers.filter(c => c.service_type === 'CC Camera').length}</strong>
            <span>CC Camera</span>
          </div>
          <div className="compact-stat-card">
            <strong>{customers.filter(c => c.service_type === 'Broadband').length}</strong>
            <span>Broadband</span>
          </div>
        </div>
      </div>
      ...
      {filteredCustomers.length === 0 ? (
        <div className="empty-state-card">
          <MdPerson style={{ fontSize: 48, color: '#d1d5db' }} />
        ...
        <>
          ...
          <div className="admin-card">
            <h3 style={{ margin: '0 0 16px' }}>Customer List ({filteredCustomers.length})</h3>
            <div className="table-wrapper" style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid #e5e7eb', background: '#f9fafb' }}>
                    <th style={{ padding: 12, textAlign: 'left', fontWeight: 700 }}>Name</th>
                    <th style={{ padding: 12, textAlign: 'left', fontWeight: 700 }}>Service</th>
                    <th style={{ padding: 12, textAlign: 'left', fontWeight: 700 }}>Email</th>
                    <th style={{ padding: 12, textAlign: 'left', fontWeight: 700 }}>Phone</th>
                    <th style={{ padding: 12, textAlign: 'left', fontWeight: 700 }}>User ID</th>
                    <th style={{ padding: 12, textAlign: 'left', fontWeight: 700 }}>Location</th>
                    <th style={{ padding: 12, textAlign: 'left', fontWeight: 700 }}>Joined</th>
                    <th style={{ padding: 12, textAlign: 'center', fontWeight: 700 }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={customer.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                      ...
                      <td style={{ padding: 12 }}>
                        <span style={{ 
                          ...
                          fontSize: 12,
                          fontWeight: 700,
                          background: customer.service_type === 'Broadband' ? '#dcfce7' : '#dbeafe',
                          color: customer.service_type === 'Broadband' ? '#166534' : '#1e40af'
                        }}>
                          {customer.service_type === 'Broadband' ? <FaWifi /> : <FaVideo />}
                          {customer.service_type}
                        </span>
                      </td>
                      <td style={{ padding: 12, fontSize: 14 }}>{customer.email}</td>
                      <td style={{ padding: 12, fontSize: 14 }}>{customer.phone || '—'}</td>
                      <td style={{ padding: 12, fontSize: 14 }}>
                        <code style={{ background: '#f3f4f6', padding: '2px 6px', borderRadius: 4 }}>
                          {customer.user_id || '—'}
                        </code>
                      </td>
                      <td style={{ padding: 12, fontSize: 14 }}>{customer.city || '—'}</td>
                      <td style={{ padding: 12, fontSize: 14 }}>
          ...
          {/* Card View */}
          <div className="customers-grid" style={{ marginTop: 24 }}>
            {filteredCustomers.map(customer => (
